...
exports.routeSchemaToSwaggerSpec = (paths, definitions, routePath, route, options = {}) => {
  const { alias, method = ['GET'], meta = {}, validate = {} } = route
  const swaggerPath = swaggerizePath(routePath)
  paths[swaggerPath] = paths[swaggerPath] || {}

  if (alias) {
    const aliasPath = swaggerizePath(alias)
    paths[swaggerPath] = {
      '$ref': '#/paths/~1' + aliasPath.split('/').filter((item) => item !== '').join('~1')
    }
